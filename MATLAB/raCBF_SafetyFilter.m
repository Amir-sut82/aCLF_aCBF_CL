function [u, theta_hat_dot_cbf, info] = raCBF_SafetyFilter(x, u_nom, theta_hat, params)
% Same HOCBF as the aCBF filter, but with the robustness margin
%   ||phi_h(x)|| * theta_bar  subtracted from the RHS.
    ...
    margin = norm(phi_h) * params.theta_bar;

    H    = 2*eye(4);
    f_qp = -2*u_nom;
    A    = -Lg_psi1;
    b    = Lf_psi1 + phi_h'*theta_hat + alpha2*psi1 - margin;
    ...
    cv = Lf_psi1 + phi_h'*theta_hat + Lg_psi1*u + alpha2*psi1 - margin;
